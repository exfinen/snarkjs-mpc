...
function forceUint8Array(x) {
  x.__proto__ = Uint8Array.prototype;
  return x;
}
...
async function getRandomRng(entropy) {
    ...
    hasher.update(forceUint8Array(crypto__default['default'].randomBytes(64)));
    const enc = new TextEncoder(); // always utf-8
    hasher.update(forceUint8Array(enc.encode(entropy)));
...
async function newZKey(r1csName, ptauName, zkeyName, logger) {
    ...
    async function hashHPointsChunk(offset, nPoints) {
        ...
        for (let i=0; i<result.length; i++) {
            csHasher.update(forceUint8Array(result[i][0]));
        }
    }
